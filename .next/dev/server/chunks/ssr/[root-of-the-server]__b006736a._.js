...
const api = {
    ...
    reverseMovement: async (movementId)=>{
        ...
        const reversalId = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$uuid$2f$dist$2d$node$2f$v4$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__$3c$export__default__as__v4$3e$__["v4"])();
...
const shipstationApi = {
    /**
     * Fetches orders from ShipStation using the provided credentials.
     * In a real application, this would make an HTTP request to our backend
     * which would securely securely securely hold the keys and call the ShipStation API.
     */ fetchOrders: async (apiKey, apiSecret)=>{
        // 1. Simulate network and API key validation
        await new Promise((resolve)=>setTimeout(resolve, 800));
        if (!apiKey || !apiSecret) {
            throw new Error('Authentication failed: Missing ShipStation API keys.');
        }
        // 2. Map the external ShipStation data to our internal Order schema
        const mappedOrders = mockShipStationResponse.map((ssOrder)=>{
            ...
            // Calculate mock margin
            const subtotal = ssOrder.items.reduce((acc, item)=>acc + item.quantity * item.unitPrice, 0);
            const mockMargin = subtotal * 0.3; // 30% mock margin
            return {
                id: orderId,
                channel: 'ShipStation',
                customerName: ssOrder.customerUsername,
                customerEmail: ssOrder.customerEmail,
                shippingAddress: formatAddress(ssOrder.shipTo),
                orderDate: ssOrder.orderDate,
                // Awaiting shipment in SS translates to 'New' in our system
                fulfillmentStatus: 'New',
                paymentStatus: 'Paid',
                items: mappedItems,
                timeline: [
                ...
                subtotal: subtotal,
                tax: ssOrder.taxAmount,
                fees: ssOrder.shippingAmount,
                total: ssOrder.orderTotal,
                margin: mockMargin,
                notes: ssOrder.customerNotes || undefined
            };
        });
        // 3. Return mapped orders
        return mappedOrders;
    },
    /**
     ...
     */ syncInventory: async (apiKey, apiSecret)=>{
        ...
        // Simulating success
        console.log("Mock: Pushed stock levels successfully to ShipStation.");
    }
};
}),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b006736a._.js.map